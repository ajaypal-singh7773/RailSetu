...
let masterTrains: any[] = [];
let isDataLoaded = false;
let stationToTrains: Record<string, any[]> = {};

const DAYS = ["SUN", "MON", "TUE", "WED", "THU", "FRI", "SAT"];
const MINUTES_IN_WEEK = 7 * 24 * 60;

function getMinutes(timeStr: string, dayStr: string) {
  ...
  Promise.all([
    ...
    masterTrains.forEach(train => {
      if (train.trainRoute && Array.isArray(train.trainRoute)) {
        train.trainRoute.forEach((s: any, idx: number) => {
    ...
    console.log("Master trains dataset completely loaded & indexed:", masterTrains.length);
  }).catch(err => {
...
const rawData: any = stationData;
const ALL_STATIONS = Array.isArray(rawData) 
  ? rawData.filter((p: any) => p.name && p.code)
  : (rawData.features || []).map((f: any) => f.properties).filter((p: any) => p.name && p.code);
...
const SearchCard = ({ className }: SearchCardProps) => {
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    if (!isDataLoaded) {
      alert("Please wait a moment while the highly accurate offline dataset is being prepared...");
      return;
    }
    ...
    let results = masterTrains.filter((train: any) => {
      ...
      for (const t1 of validTrainsFromX) {
        ...
        for (let i = startIndexA + 1; i < trainA.trainRoute.length; i++) {
          ...
          const matchingTrainsB = (stationToTrains[zCode] || []).filter(t2 => {
            ...
            const destY = trainB.trainRoute.slice(zIndexB + 1).find((s:any) => s.stationName?.endsWith(`- ${to}`));
    ...
    navigate(`/search?from=${from}&to=${to}&date=${dateStr}`, { state: { searchResults: results } });
  };
  ...
  return (
    <motion.div
      ...
      className={cn("glass-card p-6 md:p-8 w-full max-w-4xl mx-auto", className)}
    >
