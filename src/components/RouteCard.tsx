...
const RouteCard = ({ route, index }: RouteCardProps) => {
  ...
  const trains = route.legs.filter((leg: any) => leg.type !== "transfer");
  ...
  return (
    <motion.div
      ...
      onClick={() => navigate(`/route/${route.id}?date=${encodeURIComponent(route.date)}`, { state: { route } })}
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-3 mb-4">
            <span className={cn("px-3 py-1 rounded-full text-xs font-bold", getRiskColor(route.riskLevel))}>
          ...
          <div className="flex items-center gap-4 md:gap-8">
            <div className="text-center">
              <div className="text-2xl font-bold text-foreground">{departureLeg.departure}</div>
              <div className="text-sm font-medium text-foreground/60">{route.from.split(' ')[0]}</div>
            </div>
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-foreground">{arrivalLeg.arrival}</div>
              <div className="text-sm font-medium text-foreground/60">{route.to.split(' ')[0]}</div>
            </div>
          </div>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2 text-xs font-medium text-foreground/60 bg-slate-50 dark:bg-slate-900/50 p-3 rounded-xl border border-slate-200 dark:border-slate-800">
            {route.legs.map((leg: any, idx: number) => {
              if (leg.type === 'transfer') {
                return (
                  <div key={idx} className="flex flex-col items-center mx-2 animate-pulse">
                    <span className="text-[10px] text-orange-500 font-bold tracking-wider uppercase">Layover</span>
                    <span className="text-xs text-orange-600 dark:text-orange-400 font-bold bg-orange-500/10 px-2 py-0.5 rounded-full mt-0.5">
                      {leg.waitTime}
                    </span>
                    <span className="text-[10px] mt-1 text-foreground/40">{leg.station?.split(' ')[0]}</span>
                  </div>
                );
              }
              return (
                <div key={idx} className="flex items-center gap-1">
                  ...
                  {idx < route.legs.length - 1 && route.legs[idx+1].type !== 'transfer' && <ArrowRight className="w-3 h-3 mx-1" />}
                </div>
              );
            })}
          </div>
        ...
        <div className="flex lg:flex-col items-center lg:items-end justify-between border-t lg:border-t-0 lg:border-l border-border pt-4 lg:pt-0 lg:pl-6 gap-4">
          ...
          <div className="flex lg:flex-col items-center lg:items-end gap-4 lg:gap-2">
            <div className="flex flex-col items-center lg:items-end">
              <span className="text-xs text-foreground/50 font-medium">Reliability</span>
              <ReliabilityMeter score={route.reliabilityScore} className="scale-90 lg:scale-100" />
            </div>
            
            <div className="flex flex-col items-start lg:items-end">
              <span className="text-xs text-foreground/50 font-medium">Total Fare</span>
