...
import { ArrowLeft, Clock, Map, ShieldCheck, Info, Train, MapPin, RefreshCcw } from "lucide-react";
...
const RouteDetails = () => {
  ...
  const route = location.state?.route || DUMMY_ROUTES.find(r => r.id === id) || DUMMY_ROUTES[0];
  
  const urlParams = new URLSearchParams(window.location.search);
  const dateStr = urlParams.get('date');
  let displayDate = dateStr || new Date().toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' });

  const getStatusColor = (status: string) => {
    switch(status?.toLowerCase()) {
      case 'comfortable': return 'bg-green-500 text-white shadow-green-500/50';
      case 'risky': return 'bg-yellow-500 text-white shadow-yellow-500/50';
      case 'very risky': return 'bg-red-500 text-white shadow-red-500/50';
  ...
  return (
    <div className="w-full animate-fade-in max-w-5xl mx-auto">
      ...
      <div className="glass-card p-6 mb-8 flex flex-col md:flex-row justify-between md:items-center gap-6">
        <div>
          <Link to="/search" className="inline-flex items-center gap-2 text-primary hover:text-primary/80 mb-4 font-medium transition-colors">
          ...
          <div className="text-foreground/60 font-medium mt-1 flex items-center gap-4">
            ...
            <span className="flex items-center gap-1"><Clock className="w-3 h-3" /> {route.journeyTime}</span>
          </div>
        </div>
        
        <div className="flex items-center gap-6 border-t md:border-t-0 md:border-l border-border pt-4 md:pt-0 md:pl-6">
          <div className="flex flex-col items-center">
            <span className="text-xs text-foreground/50 font-medium mb-1">Reliability</span>
            <ReliabilityMeter score={route.reliabilityScore} />
          </div>
          <div className="flex flex-col items-end">
             <span className="text-xs text-foreground/50 font-medium mb-1">Total Fare</span>
             <span className="text-3xl font-bold text-accent">{route.totalFare}</span>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 ml-4 md:ml-6 bg-gradient-to-r from-orange-500/10 to-transparent border border-orange-500/20 rounded-xl p-4 flex items-center gap-4">
                        ...
                        <div>
                          <div className="font-bold text-foreground">🔄 Change trains at {leg.station?.split(' ')[0]}</div>
                          <div className="text-sm font-semibold text-orange-600 dark:text-orange-400 mt-0.5">Wait time: {leg.waitTime}</div>
                        </div>
                      </div>
                    </motion.div>
                  );
                  ...
                  >
                    ...
                    <div className="relative flex items-center gap-4 md:gap-6 mt-2 mb-2">
                       ...
                       <div className="ml-6 md:ml-8 flex flex-col">
                         <span className="text-xl font-bold text-foreground">{leg.from?.split(' ')[0]}</span>
                         <span className="text-sm font-bold text-primary">Departs {leg.departure}</span>
                       </div>
                    </div>
                    ...
                    <div className="relative ml-8 md:ml-10 my-4 py-2">
                       <div className="glass p-4 rounded-2xl inline-flex flex-col gap-3 border border-border/50 hover:border-primary/30 transition-colors shadow-sm bg-white/40 dark:bg-slate-800/40">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                               <div className="font-bold text-sm text-foreground">{leg.trainName}</div>
                               <div className="text-xs font-semibold text-foreground/50">Train {leg.trainNumber}</div>
                            </div>
                          </div>
                          
                          <div className="flex items-center gap-4 text-xs font-medium text-foreground/60 bg-background/50 p-2 rounded-lg">
                             <div className="flex items-center gap-1.5"><Clock className="w-3.5 h-3.5" /> {leg.duration}</div>
                             <div className="w-1 h-1 rounded-full bg-border"></div>
                             <div className="flex items-center gap-1.5"><MapPin className="w-3.5 h-3.5" /> {leg.distance}</div>
                          </div>
                       </div>
                    </div>
                    ...
                    <div className="relative flex items-center gap-4 md:gap-6 mb-2 mt-2">
                       ...
                       <div className="ml-6 md:ml-8 flex flex-col">
                         <span className="text-xl font-bold text-foreground">{leg.to?.split(' ')[0]}</span>
                         <span className="text-sm font-bold text-accent">Arrives {leg.arrival}</span>
                       </div>
                    </div>
                  </motion.div>
                );
        ...
        <div className="w-full lg:w-1/3 flex flex-col gap-6">
          <div className="glass-card p-6">
            ...
            <p className="text-sm text-foreground/70 mb-4 leading-relaxed">
              Based on historical data, this route has a {route.reliabilityScore}% success rate for catching all connections.
            </p>
            <div className="w-full bg-border h-2 rounded-full overflow-hidden">
               <div className="bg-green-500 h-full" style={{ width: `${route.reliabilityScore}%` }}></div>
            </div>
          </div>
          ...
          <div className="glass-card p-6">
            ...
            <ul className="space-y-3 text-sm text-foreground/70">
              <li className="flex items-start gap-2">
                ...
                Book tickets early as {route.legs[0]?.trainName} has high waitlist probability.
              </li>
              <li className="flex items-start gap-2">
                <span className="w-1.5 h-1.5 rounded-full bg-accent mt-1.5 shrink-0"></span>
                The transfer at New Delhi might require moving between platforms 1 and 12.
              </li>
            </ul>
          </div>
